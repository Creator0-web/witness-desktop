...
> **v7.59.2 Quick Tasks + Protection Toggle:** Activity Forge now includes a dedicated **Quick Tasks** card for up to five small one-off tasks at a time. Every queued item uses one shared XP value configured in Settings; click a task to award that XP and remove it, or use × to remove it without scoring. The queue is local-first and included in WITNESS Sync V1, while the underlying XP still goes through the canonical immutable ledger. Settings -> Protection also has a persistent per-device **Protection ON/OFF** control so Rapid Screen Guard + title drift tracking can be paused without changing frozen Layer 1. Re-enabling creates a fresh runtime generation so stale callbacks from the stopped guard cannot trigger a browser shutdown.
>
