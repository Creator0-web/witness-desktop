...
def award_activity_xp(activity_name, amount):
    """Award the exact XP configured for a manually checked Activity.

    Manual Activities are intentionally deterministic: if the person sets an
    activity to 50 XP, checking it adds exactly 50 XP. The older award_xp()
    path still keeps streak multipliers / random bonus rolls for the parts of
    the game that use them, but the checklist itself no longer guesses or
    mutates the configured value.
    """
    try:
    ...
    p["daily_tasks_awarded"] = p.get("daily_tasks_awarded", 0) + 1
    p["tasks_completed"] = p.get("tasks_completed", 0) + 1
    p.get("needs", {})["discipline"] = min(
        100, p.get("needs", {}).get("discipline", 0) + 8)
    _log(p, amount, f"activity:{activity_name}")
    _update_streak(p)
    _check_achievements(p)
    _save(p)
    return f"+{amount} XP"
...
def remove_activity_xp(activity_name, amount):
    """Reverse a manually checked Activity when its box is unchecked."""
    try:
        amount = max(0, int(amount))
    except (TypeError, ValueError):
        amount = 0
    ...
    p["daily_tasks_awarded"] = max(0, p.get("daily_tasks_awarded", 0) - 1)
    p["tasks_completed"] = max(0, p.get("tasks_completed", 0) - 1)
    _log(p, -amount, f"activity_unchecked:{activity_name}")
    _save(p)
    return f"-{amount} XP"
