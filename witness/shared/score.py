"""Focus score (0-100) based primarily on manual Activity completion,
...
def today_score() -> int:
    """Score = activity completion (0-100) modified by drift behavior."""
    tasks = data.get_tasks()
    now = datetime.now().strftime("%H:%M")

    if not tasks:
        return 0  # no configured Activities = 0%, not 100%

    # Activity completion: every configured Activity counts toward today
    due_count = 0
    done_count = 0
    future_count = 0
    for t in tasks:
    ...
    # base: percentage of all configured Activities done
