...
- The v7.45 stylesheet explicitly makes QLabel backgrounds transparent. This
  ...
  component needs one.
- **v7.58.2 webcam A/V-sync contract:** Windows acceptance testing isolated a noticeable constant lip-sync offset to the direct **Webcam + Mic** path; both screen-capture paths were already synchronized. Keep those working screen paths untouched. Camera-only may use source-native/optimal recorder frame cadence (`QMediaRecorder` frame rate 0) and a short pre-record camera settle period before `record()` establishes timestamps. This is recorder delivery work only; it must not alter `core/`, scoring, calendar storage, or SOS intervention behavior. If a constant offset remains after this low-risk correction, the next escalation is explicit camera-frame timestamp compensation/custom frame input, not retuning screen recording or Layer 1.
